...
def load(df, tbl):
    try:
        ...
        # save to s3
        upload_file_bucket = 'etl-sql-s3786'
        upload_file_key = 'public/' + str(tbl) + f"/{str(tbl)}"
        filepath =  upload_file_key + ".csv"
        #
        s3_client = boto3.client('s3', aws_access_key_id=access_key, aws_secret_access_key=secret_access_key,region_name='ap-southeast-3')
